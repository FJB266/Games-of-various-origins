...
// ─── RESPONSIVE SCALING ──────────────────────────────────────
// Internal game resolution stays fixed at W×H (all physics/collision code
// depends on this). We only scale how big the canvas is DRAWN on the page,
// via CSS width/height, so nothing else in the game needs to change.
const VIEWPORT_PCT = 0.9; // canvas will be up to 90% of the smaller viewport dimension
function resizeCanvas(){
  const maxW = window.innerWidth * VIEWPORT_PCT;
  const maxH = window.innerHeight * VIEWPORT_PCT;
  const scale = Math.min(maxW / W, maxH / H); // fit within viewport, keep aspect ratio
